...
class AllMemoriesMobileView extends StatefulWidget {
  const AllMemoriesMobileView({super.key});

  @override
  State<AllMemoriesMobileView> createState() => _AllMemoriesMobileViewState();
}

class _AllMemoriesMobileViewState extends State<AllMemoriesMobileView> {
  ...
  Future<void> _loadInitialMemories() async {
    ...
    try {
      final memories =
      await memoriesCrudService.fetchNextMemoriesPage(albumLimit: 10);
      setState(() {
        _memories = memories;
        _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    final groupKeys = groupedMemories.keys.toList()
      ..sort((a, b) => b.compareTo(a)); // Plus récents d'abord
        ...
        children: [
          ...
          ElevatedButton.icon(
            ...
            label: Text(_selectedDate != null
                ? DateFormat.yMMMM('fr').format(_selectedDate!)
                : 'Filtrer par date'),
          ),
